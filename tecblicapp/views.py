    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for i in inv_obj:
                ...
                qty=i.quantity
                client_name=i.client.clientName
            ...
            if generate_or_mail=='only_generate':    
                ...
                pdf = render_to_pdf('tecblicapp/inv_edit.html', filename,{'invoice':inv_obj,'igst':igst,'sgst':sgst,'cgst':cgst,'final_amt':final_amt,'gross_amount_words': num2words(final_amt, to='cardinal', lang='en_IN')})
                if pdf:
                    filename = f"{client_name}.pdf"
                    response = HttpResponse(pdf, content_type='application/pdf')
                    content = f"attachment; filename={filename}"
